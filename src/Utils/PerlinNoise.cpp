#include "Utils/PerlinNoise.h"

PerlinNoise::PerlinNoise()
{
	// Initialize the permutation vector with the reference values
	m_perm = {
		151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233,
		7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23,
		190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219,
		203, 117, 35, 11, 32, 57, 177, 33, 88, 237, 149, 56, 87, 174,
		20, 125, 136, 171, 168, 68, 175, 74, 165, 71, 134, 139, 48,
		27, 166, 77, 146, 158, 231, 83, 111, 229, 122, 60, 211, 133,
		230, 220, 105, 92, 41, 55, 46, 245, 40, 244, 102, 143, 54, 65,
		25, 63, 161, 1, 216, 80, 73, 209, 76, 132, 187, 208,  89, 18,
		169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198,
		173, 186, 3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147,
		118, 126, 255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17,
		182, 189, 28, 42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154,
		163, 70, 221, 153, 101, 155, 167, 43, 172, 9, 129, 22, 39, 253,
		19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104, 218, 246,
		97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191, 179, 162, 241,
		81, 51, 145, 235, 249, 14, 239, 107, 49, 192, 214,  31, 181, 199,
		106, 157, 184, 84, 204, 176, 115, 121, 50, 45, 127, 4, 150, 254,
		138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195,
		78, 66, 215, 61, 156, 180};
	
	// Duplicate the permutation vector
	m_perm.insert(m_perm.end(), m_perm.begin(), m_perm.end());
}

PerlinNoise::PerlinNoise(unsigned int seed)
{
	m_perm.resize(256);

	// Initialize random engine using standard mersenne twister
	// seeded with randomDevice
	std::mt19937 gen(seed);

	// Fill permutation vector with values from 0 to 255
	std::iota(m_perm.begin(), m_perm.end(), 0);

	// Shuffle using random engine
	std::shuffle(m_perm.begin(), m_perm.end(), gen);

	// Duplicate the permutation vector to avoid buffer overflow
	m_perm.insert(m_perm.end(), m_perm.begin(), m_perm.end());
}

float PerlinNoise::noise(const Vec3 & p) const
{
	// Find the unit cube that contains the point
	int X = static_cast<int>(floorf(p.x())) & 255;
	int Y = static_cast<int>(floorf(p.y())) & 255;
	int Z = static_cast<int>(floorf(p.z())) & 255;
	
	// Find relative x, y, z of point in cube
	float x = p.x() - floorf(p.x());
	float y = p.y() - floorf(p.y());
	float z = p.z() - floorf(p.z());

	// Compute fade curves for each x, y, z
	const float u = fade(x);
	const float v = fade(y);
	const float w = fade(z);

	// Hash coordinates of the 8 cube corners
	int A = m_perm[X] + Y;
	int AA = m_perm[A] + Z;
	int AB = m_perm[A + 1] + Z;
	int B = m_perm[X + 1] + Y;
	int BA = m_perm[B] + Z;
	int BB = m_perm[B + 1] + Z;

	// The gradient function calculates the dot product between
	// a pseudorandom gradient vector and the vector from the input
	// coordinate to the 8 surrounding points in its unit cube
	float a = grad(m_perm[AA    ], x    , y    , z    );
	float b = grad(m_perm[BA    ], x - 1, y    , z    );
	float c = grad(m_perm[AB    ], x    , y - 1, z    );
	float d = grad(m_perm[BB    ], x - 1, y - 1, z    );
	float e = grad(m_perm[AA + 1], x    , y    , z - 1);
	float f = grad(m_perm[BA + 1], x - 1, y    , z - 1);
	float g = grad(m_perm[AB + 1], x    , y - 1, z - 1);
	float h = grad(m_perm[BB + 1], x - 1, y - 1, z - 1);

	// Add blended results from 8 corners of cube based on the faded values
	float result = lerp(
		w,
		lerp(v, lerp(u, a, b), lerp(u, c, d)),
		lerp(v, lerp(u, e, f), lerp(u, g, h))
	);

	// We bound result to 0 - 1 for convenience
	return result * 0.5f + 0.5f;
}

float PerlinNoise::octaveNoise(const Vec3 & p, int octaves) const
{
	float total = 0.f;
	float frequency = 1.f;
	float amplitude = 1.f;
	float maxValue = 0.f; // Used for normalizing result to 0.0 - 1.0

	for (int i = 0; i < octaves; ++i)
	{
		total += noise(p * frequency) * amplitude;
		
		maxValue += amplitude;
		
		amplitude *= 0.5f;
		frequency *= 2.f;
	}

	return total / maxValue;
}

float PerlinNoise::fade(float t) const
{
	return t * t * t * (t * (t * 6 - 15) + 10);
}

// Linear interpolate
float PerlinNoise::lerp(float t, float a, float b) const
{
	return a + t * (b - a);
}

float PerlinNoise::grad(int hash, float x, float y, float z) const
{
	// Convert lower 4 bits of hash into 12 gradient directions
    switch(hash & 0xF)
    {
        case 0x0: return  x + y;
        case 0x1: return -x + y;
        case 0x2: return  x - y;
        case 0x3: return -x - y;
        case 0x4: return  x + z;
        case 0x5: return -x + z;
        case 0x6: return  x - z;
        case 0x7: return -x - z;
        case 0x8: return  y + z;
        case 0x9: return -y + z;
        case 0xA: return  y - z;
        case 0xB: return -y - z;
        case 0xC: return  y + x;
        case 0xD: return -y + z;
        case 0xE: return  y - x;
        case 0xF: return -y - z;
        default: return 0; // never happens
    }
}
